########################################################
# ORAAH Tutorial 001 - Getting Started with HDFS       #
#                                                      #
...
# and map it with an ORAAH dfs.id object
...
# This structure can now be used for working with ORAAH algorithms and commands

# Physically it contains a CSV file in HDFS (normally named part-00000) and a 
# special ORAAH Metadata Object (called __ORCHMETA__) that keeps track of the
...
# and the ORAAH Metadata in a file named __ORCHMETA__ .
...
# is automatically pushed to the ORAAH Metadata when we first did the hdfs.put().

# On the other hand, if it is the first time ORAAH is seeing a CSV folder in HDFS, the 
...
# (more details on Spark and ORAAH in the next Tutorial)

# Once hdfs.dim() is executed successfully, the number of rows is permanently stored in the ORAAH
